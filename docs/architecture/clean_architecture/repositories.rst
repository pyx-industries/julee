...
SQLAlchemy-based storage for relational queries and transactions. Same interface, different backend. **When to use:** Complex queries, transactions, or when you need relational features.
...
Repositories store things via CRUD operations. Domain defines the protocol, infrastructure provides implementations (MinIO, Memory, PostgreSQL). No business logic—repositories persist, use cases orchestrate.
