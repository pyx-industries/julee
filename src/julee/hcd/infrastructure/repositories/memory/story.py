...
class MemoryStoryRepository(MemoryRepositoryMixin[Story], StoryRepository):
    ...
    async def list_filtered(
        ...
        if solution_slug is not None:
            results = [s for s in results if s.solution_slug == solution_slug]

        # Filter by app
        if app_slug is not None:
            app_normalized = normalize_name(app_slug)
            results = [s for s in results if s.app_normalized == app_normalized]

        # Filter by persona
        if persona is not None:
            persona_normalized = normalize_name(persona)
            results = [s for s in results if s.persona_normalized == persona_normalized]
