...
async def mcp_list_personas() -> dict:
    """List all personas - both defined and derived from stories.

    Personas can be:
    - Defined explicitly via create_persona (first-class, with metadata)
    - Derived from stories (fallback, with app/epic associations only)

    Defined personas are authoritative. Derived personas appear with warnings
    suggesting they be formally defined.

    Each persona shows:
    - is_defined: Whether explicitly defined
    - Which apps they interact with (from their stories)
    - Which epics they participate in
    - Goals, frustrations, JTBD (for defined personas)
    """
...
async def mcp_get_persona(name: str) -> dict | None:
    ...
    Prefers defined personas, falls back to derived if no definition exists.
    To add a new persona, use create_persona or create stories with that persona name.
...
async def mcp_create_persona(
    ...
    """Create a first-class persona definition.

    This is the top-down approach: define personas before writing stories.
    Stories should reference the persona's name (case-insensitive match).

    Example:
        create_persona(
            slug="solutions-developer",
            name="Solutions Developer",
            goals=["Build reliable production systems", "Minimize boilerplate"],
            frustrations=["Reinventing audit patterns", "Lack of visibility"],
            jobs_to_be_done=["Deploy workflow to production", "Debug failed execution"],
            context="Enterprise developer with compliance requirements"
        )
    ...
    Args:
        ...
        jobs_to_be_done: Jobs-to-be-done for this persona
...
async def mcp_update_persona(
    ...
    """Update an existing persona definition. Only provided fields are changed.

    Note: List fields (goals, frustrations, jobs_to_be_done) replace entirely
    if provided. To add to a list, get the persona first and merge.

    Args:
        slug: Persona identifier to update
        name: New display name (optional)
        goals: New goals - replaces existing (optional)
        frustrations: New frustrations - replaces existing (optional)
        jobs_to_be_done: New JTBD - replaces existing (optional)
...
@mcp.tool()
async def mcp_delete_persona(slug: str) -> dict:
    """Delete a persona definition by slug.

    Note: This only removes the explicit definition. Stories referencing
    this persona will continue to work with a derived persona, but will
    show reconciliation warnings.

    Args:
        slug: Persona identifier to delete
    """
    return await delete_persona(slug)
