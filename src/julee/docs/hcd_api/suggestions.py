...
def story_references_undefined_persona(
    story_slug: str, persona: str, defined_personas: list[str]
) -> Suggestion:
    """Warn that a story references a persona that hasn't been defined.

    This is a reconciliation warning - stories should reference personas
    that have been formally defined via define-persona directive or MCP tool.
    """
    if defined_personas:
        action = f"Define persona '{persona}' or use an existing one: {', '.join(defined_personas[:5])}"
    else:
        action = f"Define persona '{persona}' using create_persona tool or define-persona directive"
    return Suggestion(
        severity=SuggestionSeverity.WARNING,
        category=SuggestionCategory.MISSING_REFERENCE,
        message=f"Story references undefined persona '{persona}'",
        action=action,
        tool="mcp_create_persona",
        context={
            "story_slug": story_slug,
            "persona": persona,
            "defined_personas": defined_personas[:10],
        },
    )


def persona_has_no_stories(
    persona_name: str, persona_slug: str
) -> Suggestion:
    """Warn that a defined persona has no stories referencing them.

    This is a reconciliation warning - defined personas should have
    at least one story where they appear.
    """
    return Suggestion(
        severity=SuggestionSeverity.WARNING,
        category=SuggestionCategory.ORPHAN,
        message=f"Defined persona '{persona_name}' has no stories referencing them",
        action=f"Create stories for persona '{persona_name}' or remove the persona definition if unused",
        tool="create_story",
        context={
