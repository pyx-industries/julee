...
class TestExtractAssembleDataUseCase:
    ...
    async def test_schema_passed_in_metadata(
        ...
        async def mock_execute_query(
            config, query_text, service_file_ids, query_metadata, assistant_prompt
        ):
        ...
        try:
            ...
            # Verify schema is passed in metadata
            assert "output_schema" in call["query_metadata"]
            expected_schema_section = {
                "type": "string"
            }  # Schema section for /properties/title
            assert call["query_metadata"]["output_schema"] == expected_schema_section
