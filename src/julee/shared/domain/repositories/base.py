"""Base repository protocol for julee accelerators.

Defines the generic repository interface following clean architecture
...
class BaseRepository(Protocol[T]):
    """Generic base repository protocol for domain entities.

    This protocol defines the common interface shared by all domain
    repositories across julee accelerators. It uses generics to provide
    type safety while eliminating code duplication.
    ...
    layers can provide sync adapters where needed (e.g., for Sphinx directives).
    """
