...
Use case for listing service protocols across bounded contexts.
"""
...
class ListServiceProtocolsUseCase:
    ...
    async def execute(
        ...
        """List service protocols across bounded contexts.

        Args:
            request: Request with optional bounded_context filter

        Returns:
            Response containing list of service protocols with their bounded context
        """
        ...
        # Extract service protocols from each context
        artifacts = []
        for ctx in contexts:
